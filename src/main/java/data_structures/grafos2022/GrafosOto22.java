...
public class GrafosOto22 extends JFrame{
    ...
    private AtomicBoolean directedAtBool, modifiedAtBool;
    private JTextField nodeOTField, nodeDTField, pesoTField;
    ...
    private void initComponentes(){
        ...
        modifiedAtBool 	= new AtomicBoolean(false);
        ...
        nodeOTField		= new JTextField();
        nodeDTField		= new JTextField();
        ...
        add(nodo1Label);
        nodeOTField.setBounds(20, 75, 70, 25);
        add(nodeOTField);
        nodo2Label.setBounds(100, 50, 70, 25);
        add(nodo2Label);
        nodeDTField.setBounds(100, 75, 70, 25);
        add(nodeDTField);
        ...
        createAristaB.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){
                try{
                    ...
                    if(nodesArray.size() < Integer.parseInt(nodeOTField.getText()) || Integer.parseInt(nodeOTField.getText()) < 1){
                        JOptionPane.showMessageDialog(null, "El nodo origen debe existir", "WARNING", JOptionPane.WARNING_MESSAGE);
                        nodeOTField.setText("");
                        nodeOTField.requestFocus();
                        return;
                    }
                    if(nodesArray.size() < Integer.parseInt(nodeDTField.getText()) || Integer.parseInt(nodeDTField.getText()) < 1){
                        JOptionPane.showMessageDialog(null, "El nodo destino debe existir", "WARNING", JOptionPane.WARNING_MESSAGE);
                        nodeDTField.setText("");
                        nodeDTField.requestFocus();
                        return;
                    }
                    if(Integer.parseInt(nodeOTField.getText()) == Integer.parseInt(nodeDTField.getText())){
                    ...
                    aristaTemp.setOrigen(Integer.parseInt(nodeOTField.getText()));
                    aristaTemp.setDestino(Integer.parseInt(nodeDTField.getText()));
        ...
        ereaseNodeB.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){
                try {
                    String buttonsJOPTemp[] = {"Sí","No"};
                    int nodeValueTemp = Integer.parseInt(nodeOTField.getText());
		...
		matrixItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){
                if(nodesArray.isEmpty()){
                    JOptionPane.showMessageDialog(null, "Grafo vacío", "WARNING", JOptionPane.WARNING_MESSAGE);
                    return;
                }
                costMatrixInit();
        ...
        floydItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){
                ...
                areaTArea.append(" ");
                for(Nodo nod : nodesArray){
                    printing = String.format(formating, nod.getDato());
                    areaTArea.append(printing);
                }
                areaTArea.append("\n");
                for(int i = 0; i < nodesArray.size(); i++){
            ...
            public void actionPerformed(ActionEvent e){
                if(!nodesArray.isEmpty() && (modifiedAtBool.get() || panelGraph.isModified()))
                    if (JOptionPane.showConfirmDialog( null,"¿Guardar progreso?","Grafos Otoño 2022", JOptionPane.YES_NO_OPTION,JOptionPane.WARNING_MESSAGE) == JOptionPane.YES_OPTION){
                        saveItem.doClick();
                        if(modifiedAtBool.get() || panelGraph.isModified())
                            return;
                    }
                ...
                JFileChooser fChooser = new JFileChooser(){
                    ...
                    public void approveSelection(){
                        ...
                        if(fileTemp.exists() && getDialogType() == SAVE_DIALOG){
                            int result = JOptionPane.showConfirmDialog(this,"¿Sobre escribir archivo existente?","Existing tempFile", JOptionPane.YES_NO_CANCEL_OPTION);
                            switch(result){
                ...
                try {
                    ...
                    while(fReader.hasNext()){
                        if(fReader.next().equals("n")){
                            Nodo n = new Nodo();
                            n.setX(fReader.nextInt());
                            n.setY(fReader.nextInt());
                            n.setDato(fReader.nextInt());
                            nodesArray.add(n);
                        }
                    }
                    fReader.close();
                    ...
                    panelGraph.repaint();
                    if(nodesArray.isEmpty()){
                        JOptionPane.showMessageDialog(panelGraph, "No se encontraron nodesArray en este archivo", "Grafos Otoño 2022", JOptionPane.WARNING_MESSAGE);
                        fileSelected = null;
                    }
                    modifiedAtBool.set(false);
                    panelGraph.setModified(false);
